...
App Streamlit pour le centre de formation féminin du Paris FC (suivi GPS, tagging vidéo tactique, médical, présence, recrutement). Le code est concentré dans un seul gros fichier applicatif, avec quelques scripts satellites indépendants qui tournent hors de l'app (cron/systemd) et écrivent dans Supabase.
...
- `drive_utils.py` — helpers Drive minimalistes utilisés uniquement par `sync_drive_to_supabase.py` (pas par l'app).
...
- Remote GitHub : `PFCCDFF/parisfc`. Déploiement automatique via `.github/workflows/deploy.yml` : tout push sur `main` déclenche une action SSH qui fait `cd /opt/parisfc && git pull && systemctl restart parisfc` sur le VPS.
...
- L'app Streamlit lit `st.secrets` (`.streamlit/secrets.toml`, gitignored) : `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`, et un bloc `[GOOGLE_SERVICE_ACCOUNT_JSON]`.
- `sync_drive_to_supabase.py` et `backfill_joueuse_tags.py` lisent un **`.env` séparé** (python-dotenv) : `SUPABASE_URL`, `SUPABASE_SERVICE_KEY`.
...
Les rôles/permissions sont gérés via un fichier Excel synchronisé depuis Drive (`Classeurs permissions streamlit.xlsx`), pas en base de données. `get_user_role()` distingue `ROLE_ADMIN` / `ROLE_STAFF` / `ROLE_JOUEUSE`, ce qui détermine les onglets visibles dans `script_streamlit()`. Le profil "Staff Pro" a un onglet dédié forcé comme page d'accueil à la connexion.
